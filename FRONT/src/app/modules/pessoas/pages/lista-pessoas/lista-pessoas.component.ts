...
export class ListaPessoasComponent implements OnInit, AfterViewInit {
  ...
  onCopiarLinkCadastro(): void {
    const registrationUrl = `${window.location.origin}/cadastro-cliente`;
    navigator.clipboard.writeText(registrationUrl).then(
      () => {
        this.toastService.showSuccess('Link de auto-cadastro copiado! Compartilhe no WhatsApp com suas clientes.');
      },
      (err) => {
        console.error('Erro ao copiar link:', err);
        this.toastService.showError('Não foi possível copiar o link automaticamente. URL: ' + registrationUrl);
      }
    );
  }
