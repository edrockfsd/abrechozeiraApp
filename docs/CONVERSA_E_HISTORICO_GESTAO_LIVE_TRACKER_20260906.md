...
| Isolamento de Lives (Peça 106) | "Ontem, um dos códigos apresentados foi o 106... o sistema estava me mostrando registros do dia 31/08/2026 ao invés dos de ontem (06/09/2026)" | **Correção de Isolamento Estrito por LiveSessionId**: Identificada dupla falha: 1) No frontend (`gestao-live.component.ts`), `carregarDadosLive()` chamava genericamente `/api/LiveSessions` e pegava `sessions[0]`, que em produção retornava a sessão antiga de 31/08 (`18228017698323620`). 2) No backend (`LiveTrackerService.cs`), quando `liveVideoId` não batia ou vinha zerado, `ObterAuditoriaAsync` executava um fallback global perigoso sem filtro de live (`db.ComentarioLive.OrderBy(c => c.CreatedAt)...`), trazendo os comentários mais antigos da história do banco. **Solução implementada**: Criação do endpoint contextual `GET /api/LiveTracker/info/{liveId}` que descobre e vincula o `LiveVideoId` autêntico daquela live específica (via memória, Regex de `Live.Observacoes` ou data). Eliminação completa de qualquer consulta ou fallback global no banco em `LiveTrackerService.cs`: consultas retroativas e de auditoria agora são **estritamente filtradas por `c.LiveSessionId == vidFinal.Value`**, garantindo 100% de isolamento entre lives passadas e atuais. |
