...
import { SuperfreteService } from '../../../envios/services/superfrete.service';
import { CotacaoFreteResultado, CriarEtiquetaResultado, SUPERFRETE_STATUS_MAP } from '../../../envios/interfaces/superfrete.interface';
...
export class CadastroPedidoComponent implements OnInit {
  ...
  public clienteCpf = '';
  public clienteEmail = '';
  public fretePeso: number = 0.3;
  public fretefAltura: number = 5;
  public freteLargura: number = 11;
  public freteComprimento: number = 16;
  public cotandoFrete = false;
  public gerandoEtiqueta = false;
  public cotacaoResultados: CotacaoFreteResultado[] = [];
  public cotacaoSelecionada: CotacaoFreteResultado | null = null;
  public etiquetaGerada: (CriarEtiquetaResultado & { statusLabel?: string }) | null = null;
  public get enderecoSelecionadoParaFrete(): any {
    const endId = this.pedidoForm.get('enderecoId')?.value;
    return endId ? this.enderecos.find(e => e.id === endId) : null;
  }
  ...
  private initForm(): void {
    ...
    // Observa mudanças no clienteId para carregar os endereços e dados completos
    this.pedidoForm.get('clienteId')?.valueChanges.subscribe(clienteId => {
      ...
      if (clienteId) {
        ...
        this.carregarDadosClienteFull(clienteId);
      } else {
        this.enderecos = [];
        this.clienteCpf = '';
        this.clienteEmail = '';
  ...
  private carregarEnderecos(clienteId: number): void {
    ...
    this.enderecoService.listarPorPessoa(clienteId).subscribe({
      next: (enderecos) => {
        ...
        // Auto-selecionar o CEP se o endereço já estiver preenchido no formulário
        const endId = this.pedidoForm.get('enderecoId')?.value;
        if (endId) {
          const selectedEnd = this.enderecos.find(e => e.id === endId);
          if (selectedEnd && selectedEnd.cep) {
            let cep = selectedEnd.cep.replace(/\D/g, '');
            if (cep.length > 5) {
              cep = cep.slice(0, 5) + '-' + cep.slice(5, 8);
            }
            this.freteCepDestino = cep;
          }
        }
      },
      error: (error) => {
  ...
  private carregarDadosClienteFull(clienteId: number): void {
    this.pessoaService.buscarPorId(clienteId).subscribe({
      next: (pessoa) => {
        if (pessoa) {
          this.clienteCpf = pessoa.cpf || '';
          this.clienteEmail = pessoa.email || '';
        }
      },
      error: (err) => {
  ...
  onEnderecoSelecionado(enderecoId: number): void {
    ...
    if (endereco && endereco.cep) {
      let cep = endereco.cep.replace(/\D/g, '');
      if (cep.length > 5) {
        cep = cep.slice(0, 5) + '-' + cep.slice(5, 8);
      }
      this.freteCepDestino = cep;
  ...
  private carregarClientes(): void {
    ...
    this.pessoaService.listarClientesCombo().subscribe({
      next: (clientes) => {
        this.clientes = (clientes || []).sort((a: any, b: any) => (a.nome || '').localeCompare(b.nome || ''));
      ...
      this.pedidoProdutoService.listarPorPedido(pedidoId).subscribe({
        next: (produtos) => {
          ...
          this.produtos = produtos.map(p => ({
            ...
            preco: (p as any).precoVenda || (((p.valorFinalProduto ?? 0) + (p.descontoValor ?? 0)) / p.quantidade) || 10.0,
  ...
  // ─── Superfrete: Cotação de Frete ──────────────────────────────────────────
  onCepInput(event: Event): void {
    const input = event.target as HTMLInputElement;
    let v = input.value.replace(/\D/g, '');
    if (v.length > 5) v = v.slice(0, 5) + '-' + v.slice(5, 8);
    this.freteCepDestino = v;
    input.value = v;
  }

  onCotarFrete(): void {
    if (!this.freteCepDestino || !this.fretePeso) {
      this.toastObj.show({ title: 'Atenção', content: 'Informe o CEP de destino e o peso do pacote.', cssClass: 'e-warning', timeOut: 3000 });
      return;
    }
    const cepLimpo = this.freteCepDestino.replace(/\D/g, '');
    if (cepLimpo.length !== 8) {
      this.toastObj.show({
        title: 'CEP Inválido',
        content: 'O CEP de destino deve conter exatamente 8 dígitos.',
        cssClass: 'e-warning',
        timeOut: 4000
      });
      return;
    }
    this.cotandoFrete = true;
    this.cotacaoResultados = [];
    this.cotacaoSelecionada = null;

    this.superfreteService.cotarFrete({
      cepOrigem: '',  // preenchido pelo backend com o config
      cepDestino: this.freteCepDestino.replace('-', ''),
      servicos: '1,2,17,3',
      peso: this.fretePeso,
      altura: this.fretefAltura || 5,
      largura: this.freteLargura || 11,
      comprimento: this.freteComprimento || 16,
    }).subscribe({
      next: (resultados) => {
        this.cotacaoResultados = resultados;
        this.cotandoFrete = false;
        if (resultados.length === 0) {
          this.toastObj.show({ title: 'Sem opções', content: 'Nenhuma transportadora disponível para esse CEP.', cssClass: 'e-warning', timeOut: 4000 });
        }
      },
      error: (err) => {
        console.error('Erro ao cotar frete:', err);
        this.toastObj.show({ title: 'Erro', content: 'Não foi possível cotar o frete. Verifique o token do Superfrete.', cssClass: 'e-error', timeOut: 4000 });
        this.cotandoFrete = false;
      }
    });
  }

  selecionarCotacao(opcao: CotacaoFreteResultado): void {
    this.cotacaoSelecionada = opcao;
    // Atualiza o campo valorFrete do formulário
    this.pedidoForm.get('valorFrete')?.setValue(opcao.price);
  }

  getClienteNome(): string {
    const cliId = this.pedidoForm.get('clienteId')?.value;
    if (!cliId) return '';
    return this.clientes.find(c => c.id === cliId)?.nome ?? '';
  }
  ...
  // ─── Superfrete: Gerar Etiqueta ────────────────────────────────────────────
  onGerarEtiqueta(): void {
    if (!this.cotacaoSelecionada || !this.enderecoSelecionadoParaFrete) return;
    const endereco = this.enderecoSelecionadoParaFrete;

    if (!this.clienteCpf) {
      this.toastObj.show({
        title: 'CPF Requerido',
        content: 'O cliente selecionado não possui CPF cadastrado. Cadastre o CPF no menu de pessoas para gerar a etiqueta.',
        cssClass: 'e-warning',
        timeOut: 5000
      });
      return;
    ...
    this.gerandoEtiqueta = true;
    this.superfreteService.criarEtiqueta({
      servicoId: this.cotacaoSelecionada.id,
      destinatarioNome: this.pedidoForm.get('clienteId')?.value
        ? (this.clientes.find(c => c.id === this.pedidoForm.get('clienteId')?.value)?.nome ?? 'Destinatario')
        : 'Destinatario',
      destinatarioEndereco: endereco.logradouro || endereco.address || '',
      destinatarioNumero: endereco.unidade || endereco.numero || '1',
      destinatarioBairro: endereco.bairro || 'NA',
      destinatarioCidade: endereco.localidade || endereco.cidade || '',
      destinatarioEstado: (endereco.estado || '').toUpperCase().slice(0, 2),
      destinatarioCep: (endereco.cep || '').replace('-', ''),
      destinatarioEmail: this.clienteEmail || undefined,
      destinatarioCpf: this.clienteCpf ? this.clienteCpf.replace(/\D/g, '') : undefined,
      peso: this.cotacaoSelecionada.packages?.[0]
        ? parseFloat(this.cotacaoSelecionada.packages[0].weight) || this.fretePeso
        : this.fretePeso,
      altura: this.cotacaoSelecionada.packages?.[0]
        ? parseFloat(this.cotacaoSelecionada.packages[0].dimensions?.height ?? '0') || this.fretefAltura
        : this.fretefAltura,
      largura: this.cotacaoSelecionada.packages?.[0]
        ? parseFloat(this.cotacaoSelecionada.packages[0].dimensions?.width ?? '0') || this.freteLargura
        : this.freteLargura,
      comprimento: this.cotacaoSelecionada.packages?.[0]
        ? parseFloat(this.cotacaoSelecionada.packages[0].dimensions?.length ?? '0') || this.freteComprimento
        : this.freteComprimento,
      produtos: this.produtos.map(p => ({
        nome: p.descricao,
        quantidade: p.quantidade,
        valorUnitario: p.preco && p.preco > 0 ? p.preco : 10.0
      }))
    }).subscribe({
      next: (resultado) => {
        const statusInfo = SUPERFRETE_STATUS_MAP[resultado.status] ?? { label: resultado.status };
        this.etiquetaGerada = { ...resultado, statusLabel: statusInfo.label };
        this.gerandoEtiqueta = false;
        this.toastObj.show({
          title: 'Etiqueta Gerada!',
          content: `Etiqueta criada no Superfrete. ID: ${resultado.id}. Valor: R$ ${resultado.price.toFixed(2)}`,
          cssClass: 'e-success',
          timeOut: 6000
        });
      },
      error: (err) => {
        ...
        this.toastObj.show({ title: 'Erro', content: 'Não foi possível gerar a etiqueta. ' + (err.error?.message ?? ''), cssClass: 'e-error', timeOut: 5000 });
        this.gerandoEtiqueta = false;
      }
    });
  }
